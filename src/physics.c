...
struct u_vec2 phys_object_collide(struct phys_object_t *phys_obj, struct vec2 phys_pos, struct obj_buf_t *obj_buf, struct u_bounds_t scene_bounds)
{
	// Null checks.
	...
	// Offsetting position for bounds to work properly.
	uint8_t pos_offset = ((phys_obj->obj->bounds.max.x - phys_obj->obj->bounds.min.x) >> 1);

	// Clamp offsets if they go past the viewable area.
	// X offsets.
	int16_t min_x_off = phys_obj->obj->bounds.min.x + phys_pos.x;
	int16_t max_x_off = phys_obj->obj->bounds.max.x + phys_pos.x;

	if (min_x_off > WIDTH + WIDTH_OFFSET) phys_pos.x = WIDTH + WIDTH_OFFSET - phys_obj->obj->bounds.min.x;
	else if (min_x_off < WIDTH_OFFSET) phys_pos.x = 0 - phys_obj->obj->bounds.min.x;

	else if (max_x_off > WIDTH + WIDTH_OFFSET) phys_pos.x = WIDTH + WIDTH_OFFSET - phys_obj->obj->bounds.max.x;
	else if (max_x_off < WIDTH_OFFSET) phys_pos.x = 0 - phys_obj->obj->bounds.max.x;

	// Y offsets.
	int16_t min_y_off = phys_obj->obj->bounds.min.y + phys_pos.y;
	int16_t max_y_off = phys_obj->obj->bounds.max.y + phys_pos.y;

	if (min_y_off > HEIGHT + HEIGHT_OFFSET) phys_pos.y = HEIGHT + HEIGHT_OFFSET - phys_obj->obj->bounds.min.y;
	else if (min_y_off < HEIGHT_OFFSET) phys_pos.y = 0 - phys_obj->obj->bounds.min.y;

	else if (max_y_off > HEIGHT + HEIGHT_OFFSET) phys_pos.y = HEIGHT + HEIGHT_OFFSET - phys_obj->obj->bounds.max.y;
	else if (max_y_off < HEIGHT_OFFSET) phys_pos.y = 0 - phys_obj->obj->bounds.max.y;


	// Update position with potential new coordinates.
	phys_obj->obj->bounds.min.x += phys_pos.x;
	phys_obj->obj->bounds.max.x += phys_pos.x;

	phys_obj->obj->bounds.min.y += phys_pos.y;
	phys_obj->obj->bounds.max.y += phys_pos.y;

	phys_obj->obj->pos.x += phys_pos.x;
	phys_obj->obj->pos.y += phys_pos.y;
	...
	for (uint8_t i = 0; i < obj_buf->filled; ++i)
	{
		...
		switch (obj_collision)
		{
			...
			case COLLIDE_DOWN:
				...
				break;
			case COLLIDE_LEFT:
				update_pos.x = obj_buf->buf[i].bounds.max.x + pos_offset;
				
				break;
			case COLLIDE_RIGHT:
				update_pos.x = obj_buf->buf[i].bounds.min.x - pos_offset;
				break;
		}
	...
	// Check for scene bounds collision after checking all objects.
	...
	{
		update_pos.x = scene_bounds.min.x + pos_offset;
	}
	if (scene_collision & COLLIDE_RIGHT)
	{
		update_pos.x = scene_bounds.max.x - pos_offset;
	}
...
void physics_tick(struct phys_obj_buf_t *phys_obj_buf, struct obj_buf_t *obj_buf, struct scene_t * scene)
{
	...
	for (uint8_t i = 0; i < phys_obj_buf->filled; ++i)
	{
		...
		struct vec2 phys_pos = { phys_obj_buf->buf[i].vel.x + phys_obj_buf->buf[i].acc.x,
								    -(phys_obj_buf->buf[i].vel.y + phys_obj_buf->buf[i].acc.y) };

		// Compute the final object position.
		struct u_vec2 new_pos = phys_object_collide(&phys_obj_buf->buf[i], phys_pos, obj_buf, scene->bounds);
